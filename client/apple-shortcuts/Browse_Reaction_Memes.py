...
for _ in range (50):
    if searchExit == FALSE:
        ...
        url = URL(f"{serverURL}/browse?query={query}&page={pageNo}&per_page={resultsPerPage}&media_type={searchMediaType}")
        ...
        text = f'''
            ...
            BEGIN:VCARD
            VERSION:3.0
            N;CHARSET=utf-8:Change Results Per Page
            ORG:Change the number of search results shown per page.
            NOTE;CHARSET=UTF-8:ChangeCount
            {searchIcon}
            END:VCARD
        ...
        text = f'''
        Browsing Reaction Memes ⸱ Page {pageNo}
        '''
        ...
        if chosenItem.Notes == 'ChangeCount':
            isControlItem = TRUE
            resultsPerPage = AskForInput(Input.Number, prompt='How many results per page?', allowDecimals=False, allowNegatives=False)
